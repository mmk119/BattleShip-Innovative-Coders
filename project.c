...
int main() {
    ...
    printf("%s's turn. Enter command (Fire [coordinate], Radar [coordinate], Smoke Screen [coordinate], or Artillery [coordinate]): ", attackingPlayer->name);
...
void SmokeScreen(Player *player, char *coordinate, Player *opponent) {
    ...
    for (int i = row; i <= row + 1; i++) {
        for (int j = col; j <= col + 1; j++) {
            opponent->grid[i][j] = 'O'; // Mark the area as obscured
        }
    }
